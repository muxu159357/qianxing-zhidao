...
允许的类型：功能、修复、文档、测试、重构、样式、构建、配置、优化、清理
...
功能(小程序): 接入景点列表后端数据
修复(后端): 修复路线详情字段映射
文档(规则): 更新连续开发硬规则与阶段复盘
测试(接口): 补充公开接口联调验证记录
```

禁止使用纯英文 commit（feat/fix/docs/chore/refactor 等），除非是第三方工具强制生成且无法避免。
...
每一轮必须遵守完整流程：
```
查看总体代码 → 查看相关文档 → 设计本轮方案 → 写代码 → 自查审核 → 测试验证 → 修复问题 → 更新记录 → 中文 commit → push → 进入下一轮
```
