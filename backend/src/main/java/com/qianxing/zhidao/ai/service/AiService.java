...
public class AiService {
    ...
    private final AiTodaySuggestionService todaySuggestionService;
    ...
    public AiService(QxAiPlanRequestMapper planRequestMapper, QxAiPlanResultMapper planResultMapper,
                     ...
                     QxUserTripMapper tripMapper, AiTodaySuggestionService todaySuggestionService,
        ...
        this.todaySuggestionService = todaySuggestionService;
    ...
    public Map<String, Object> chat(Long userId, String question, Map<String, Object> body) {
        ...
        if (todaySuggestionService.isTodaySuggestionQuestion(q)) {
    ...
    // ========== Today suggestion (delegated to AiTodaySuggestionService) ==========

    @SuppressWarnings("unchecked")
    private Map<String, Object> todaySuggestion(Long userId, String q, Map<String, Object> body) {
        Map<String, Object> tripCtx = todaySuggestionService.buildTripProgressContext(userId);
        Map<String, Object> locCtx = todaySuggestionService.buildLocationContext(body);
        String weatherCtx = buildWeatherContext(q);

        if (llmClient.isConfigured()) {
            try {
                String prompt = SYSTEM_PROMPT
                        + todaySuggestionService.buildTodayPromptPrefix(tripCtx, locCtx)
                        + "\n" + weatherCtx
        ...
        String answer = todaySuggestionService.buildFallbackAnswer(tripCtx);
        Map<String, Object> result = buildChatResult(answer, false, 0.8, q);
        result.put("tripProgressContext", tripCtx);
        result.put("locationContext", locCtx);
        return result;
    }
