...
-- Version: 2.0 | Charset: utf8mb4 | PHASE-4-A
-- 不包含外键约束，使用逻辑外键+索引
...
CREATE TABLE qx_route (
  id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
  route_code VARCHAR(32) NOT NULL COMMENT '路线编码 route-1',
...
CREATE TABLE qx_media_asset (
  id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
  biz_type VARCHAR(20) NOT NULL COMMENT 'scenic/route/theme/user_trip/knowledge',
  biz_id BIGINT NOT NULL COMMENT '业务主键ID',
  asset_type VARCHAR(20) NOT NULL COMMENT 'cover/thumb/gallery/fallback',
  ...
  width INT COMMENT '宽(px)',
  height INT COMMENT '高(px)',
...
CREATE TABLE qx_user_trip (
  ...
  route_id BIGINT COMMENT '来源路线模板(可空)',
...
CREATE TABLE qx_ai_plan_request (
  ...
  profile_json JSON COMMENT '生成的画像JSON',
...
CREATE TABLE qx_ai_plan_result (
  ...
  route_json JSON COMMENT 'AI返回的完整路线JSON',
  route_snapshot_id BIGINT COMMENT '用户采纳后对应qx_user_trip.id',
...
CREATE TABLE qx_knowledge_article (
  id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键',
  article_code VARCHAR(32) NOT NULL COMMENT '文章编码 k1',
