...
<template>
  <div class="am-page"><div class="ph"><h2 class="pt">媒体管理</h2><div class="ha"><el-select v-model="bizType" placeholder="业务类型" clearable style="width:140px" @change="onSearch"><el-option label="景点" value="scenic"/><el-option label="路线" value="route"/><el-option label="知识库" value="knowledge"/></el-select><el-button type="primary" @click="onSearch">搜索</el-button><el-button @click="load">刷新</el-button><el-button disabled>上传（需配置存储服务）</el-button></div></div>
    <el-card shadow="never"><el-alert title="文件上传需要配置 OSS/S3 等存储服务，当前只支持管理已有媒体记录" type="info" :closable="false" style="margin-bottom:16px"/>
      <el-table :data="assets" v-loading="loading" stripe><el-table-column prop="id" label="ID" width="60"/><el-table-column prop="bizType" label="业务类型" width="90"/><el-table-column prop="bizId" label="关联ID" width="80"/><el-table-column prop="assetType" label="类型" width="80"/><el-table-column prop="url" label="URL" min-width="200"><template #default="{row}"><a :href="row.url" target="_blank" style="color:#2f6bff;font-size:12px">{{ row.url }}</a></template></el-table-column><el-table-column prop="sortOrder" label="排序" width="70"/><el-table-column label="状态" width="80"><template #default="{row}"><el-tag :type="sMap[row.status]?.t||'info'" size="small">{{ sMap[row.status]?.tx||'未知' }}</el-tag></template></el-table-column><el-table-column label="操作" width="160"><template #default="{row}"><el-button text size="small" :type="row.status===1?'warning':'success'" @click="onToggle(row)">{{ row.status===1?'停用':'启用' }}</el-button><el-button text type="danger" size="small" @click="onDel(row)">删除</el-button></template></el-table-column></el-table>
      <div v-if="!loading&&assets.length===0" class="eh">暂无数据</div>
      <div style="margin-top:16px;text-align:right"><el-pagination v-model:current-page="currentPage" :page-size="pageSize" :total="total" layout="total,prev,pager,next" @current-change="(p:number)=>{currentPage=p;load()}"/></div>
    </el-card>
  </div>
</template>
